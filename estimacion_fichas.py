...
### Correo_Institucional: jquinteroa@sena.edu.co
### Correo_personal: jersson.9.5@gmail.com
...
def mostrar_estimacion_fichas():
    ...
    st.write("Agregar aquí contenido específico para la estimación de fichas.")

    # Lectura de los archivos .xlsx
    df_entrada = Cargar_archivos(key="Base Historica")
    df_leer_vigencias = Archivo_Registro_calificado(key="Registro Calificado")
    df_metas = Cargar_archivos(key="Metas Proximo año")

    if st.button("Calcular CUPOS_PROXIMO_AÑO"):
        nuevo_df = Calcular_Cupos(df_entrada,df_leer_vigencias,df_metas)
        st.write("Estimación de fichas calculado:")
        ...
        st.download_button(
            ...
            data=excel_data,
            file_name=fecha_actual_str+"_Cupos_Ofertados_AÑO_"+str(next_year)+".xlsx",
